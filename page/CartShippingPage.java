...
public class CartShippingPage extends JPanel {
	...
	public CartShippingPage (JPanel panel, Cart cart) {
		...
		radioPanel.setBounds (380, 0, 700, 50);
		radioPanel.setLayout(new FlowLayout());
		add(radioPanel);
		JLabel radioLabel = new JLabel("정보와 같습니까?");
		radioLabel.setFont(ft);
		JRadioButton radio0k = new JRadioButton("예");
		radio0k.setFont(ft);
		JRadioButton radioNo = new JRadioButton("아니요");
		radioNo.setFont(ft);
		radioPanel.add(radioLabel);
		radioPanel.add(radio0k);
		...
		radio0k.setSelected(true);
		radioNo.setSelected(false); 
		UserShippingInfo(true);
		

		this.mCart = cart;
		radio0k.addActionListener(new AbstractAction() { 
			public void actionPerformed (ActionEvent e) {
		
				if (radio0k.isSelected()) {
		...
		public void UserShippingInfo(boolean select) {
			...
			Font ft;
			ft = new Font("BE", Font.BOLD, 15);
		
			JPanel namePanel = new JPanel(); 
			namePanel.setBounds (0, 100, 700, 50); //namePanel.setBackground(Color.GRAY);
			...
			addressText.setFont(ft); addressPanel.add(addressText); 
			shippingPanel.add(addressPanel);
			JPanel buttonPanel = new JPanel(); buttonPanel.setBounds(0, 300, 700, 100);
